...
for modname in required_modules:
    try:
        ...
        if modname is not "fake_useragent":
            print(
                f"\n******* Failed to load module {modname}. Make sure you have installed correctly dependencies in requirements.txt. *******\n"
            )
            quit()
...
class InstaBot:
    ...
    def check_for_bot_update(self):
        self.write_log(" --> I'm looking for updates...\n")
        ...
        try:
            ...
            if int(updated_timestamp.text) > int(current_version_timestamp.read()):
                ...
                self.write_log(" --> INFO: to update Instabot copy and paste this command in your terminal: git pull https://github.com/instabot-py/instabot.py.git \n")
                
                self.write_log(" --> INFO: save your example.py file that will be overwritten after the update... \n")
            else:
                self.write_log(" --> You are running the latest stable version! \n")
                self.write_log(" --> I'm ready to start... \n")
                
        except:
            self.write_log(" --> INFO: couldn't check for updates...\n")
    ...
    def login(self):
        ...
        if self.session_file is not None and os.path.isfile(self.session_file):
            self.write_log(f" --> ***** I found session file {self.session_file} for this account... *****\n")
            successfulLogin = True
            with open(self.session_file, "rb") as i:
                cookies = pickle.load(i)
                self.s.cookies.update(cookies)
        else:
            self.write_log(" --> Trying to login as: {}...\n".format(self.user_login))
            ...
            if loginResponse.get("message") == "checkpoint_required":
                try:
                    ...
                    with self.s as clg:
                        ...
                        if complete_challenge.status_code != 200:
                            self.write_log("\n --> Entered code is wrong, try again later... \n")
                            return
                ...
                self.write_log(" --> Login error! Check your login data and retry!\n")
                return
        ...
        if successfulLogin:
            ...
            if finder != -1:
                ...
                log_string = f" --> Login success for user authenticated like: {self.user_login}\n"
                self.write_log(log_string)
                if self.session_file is not None:
                    self.write_log(
                        f" --> INFO: saving cookies to session file {self.session_file}\n\n"
                    )
        ...
        if self.follow_counter >= self.unfollow_counter:
            for i in range(len(self.bot_follow_list)):
                ...
                if check_already_unfollowed(self, f[0]):
                    ...
                    log_string = " --> Trying to unfollow number %s" % (f[0])
                    self.write_log(log_string)
    ...
    def get_media_id_by_tag(self, tag):
        ...
        if self.login_status:
            if tag.startswith("l:"):
                ...
                log_string = " --> Get Media by location: %s\n" % (tag)
                self.write_log(log_string)
            ...
            if self.login_status == 1:
                ...
                try:
                    ...
                    if follower > 3000 or follows > 1500:
                        self.write_log(
                            "   >>>This is probably Selebgram, business or a fake account\n"
                        )
                    if follow_viewer:
                        return None
                    return user_info
                        ...
                        if (
                            ...
                            if like != 0:
                                if like.status_code == 200:
                                    ...
                                    self.error_400 = 0
                                    self.like_counter += 1
                                    log_string = f" --> Liked: {self.media_by_tag[i]['node']['id']}."
                                    insert_media(
                                        self,
                                    ...
                                    self.write_log(log_string)
                                    log_string = f" --> Number of liked medias: {self.like_counter}"
                                    self.write_log(log_string)
                                    print("\n-------------> END LIKE SECTION <-------------")
                                elif like.status_code == 400:
                                    log_string = f" --> !!!!! NOT LIKED, ERROR CODE: {like.status_code} !!!!!"
                                    self.write_log(log_string)
                                    log_string = f" --> !!!!! INSTAGRAM HAS PROBABLY BLOCKED THE LIKE ACTION ON YOUR ACCOUNT !!!!!\n"
                                    insert_media(
                                        self,
                                        media_id=self.media_by_tag[i]["node"]["id"],
                                        status="400",
        ...
        if self.login_status:
            ...
            try:
                ...
                if follow.status_code == 200:
                    ...
                    log_string = f" --> Followed: {user_id}"
                    self.write_log(log_string)
                    log_string = f" --> Number of followed accounts: {self.follow_counter}.\n "
                    self.write_log(log_string)
                    print("---------------> END FOLLOW SECTION <---------------\n\n")
        ...
        if self.login_status:
            ...
            try:
                ...
                if unfollow.status_code == 200:
                    ...
                    log_string = f" --> Unfollowed: {user_id}"
                    self.write_log(log_string)
                    log_string = f" --> Number of unfollowed accounts: {self.unfollow_counter} \n"
                    self.write_log(log_string)
                    print("-----------------> END UNFOLLOW SECTION <-----------------\n\n\n")
    ...
    def new_auto_mod_like(self):
        ...
        try:
            ...
            print("\n !!!!! Couldn't remove media, but don't worry !!!!! \n")
    ...
    def new_auto_mod_follow(self):
        ...
        if (
            ...
            if self.user_min_follow != 0 or self.user_max_follow != 0:
                try:
                    ...
                    if followers < self.user_min_follow:
                        print()
                        self.write_log(
                            f" --> INFO: Won't follow {username}: does not meet user_min_follow requirement...\n"
                        )
                        return
                    ...
                    if self.user_max_follow != 0 and followers > self.user_max_follow:
                        print()
                        self.write_log(
                            f" --> INFO: Won't follow {username}: does not meet user_max_follow requirement...\n"
                        )
                        return
            ...
            if (
                ...
                print()
                self.write_log(
                    f" --> INFO: Already followed before {self.media_by_tag[0]['node']['owner']['id']}\n"
                )
    ...
    def populate_from_feed(self):
        ...
        try:
            ...
            self.write_log(" --> INFO: could not populate from recent feed. \n")
        ...
        if time.time() > self.next_iteration["Unfollow"] and self.unfollow_per_day != 0:
            ...
            if self.bot_mode == 0 or self.bot_mode == 3:
                ...
                try:
                    ...
                    self.write_log(
                        " -->INFO: Could not populate from recent feed right now... \n"
                    )
                print()
                print("\n---------------> DATABASE UNFOLLOW SECTION <---------------\n")
                log_string = f" --> Trying to unfollow number {self.unfollow_counter + 1}"
                self.write_log(log_string)
                self.auto_unfollow()
        ...
        try:
            ...
            if check_comment.status_code == 200:
                ...
                if "dialog-404" in check_comment.text:
                    self.write_log(
                        f" --> INFO: Tried to comment {media_code} but it doesn't exist (404). Resuming... \n"
                    )
                    del self.media_by_tag[0]
                    return True
        ...
        while checking:
            ...
            if media_to_unlike:
                ...
                if request.status_code == 200:
                    ...
                    self.write_log(" -->INFO: couldn't unlike media, resuming... \n")
                    checking = False
            else:
                self.write_log(" --> INFO: no medias to unlike...\n")
                checking = False
    ...
    def auto_unfollow(self):
        ...
        while checking:
            ...
            if not username_row:
                self.write_log(" --> INFO: looks like there is nobody to unfollow...\n")
                print("-----------------> END UNFOLLOW SECTION <------------------\n\n\n")
                return False
            ...
            if not current_user:
                log_string = "\n --> INFO: API limit reached from Instagram... Will try later...\n"
                self.write_log(log_string)
                return False
            for wluser in self.unfollow_whitelist:
                if wluser == current_user:
                    log_string = "\n --> INFO: found whitelist user, starting search again...\n"
                    self.write_log(log_string)
                    break
            else:
                checking = False
        ...
        if self.login_status:
            ...
            if self.login_status == 1:
                ...
                try:
                    ...
                    logging.exception(" --> INFO: Except on auto_unfollow! \n")
                    time.sleep(3)
                    return False
            else:
                return False
            
            #If the account follow me and I follow this account everything is ok
            #so nothing happen...
            if (
                self.is_active_user is True
                and self.is_follower is True
                and self.is_following is True
            ):
                 print("\n---------------> END UNFOLLOW SECTION <---------------\n\n")
            ...
            if (
                ...
                print()
                self.write_log(" --> INFO: "+ current_user + " will be unfollowed...")
    ...
    def get_media_id_recent_feed(self):
        if self.login_status:
            print()
            now_time = datetime.datetime.now()
            log_string = f" --> Ehi {self.user_login}, I get media ID from your recent feed..."
            self.write_log(log_string)
            if self.login_status == 1:
                ...
                try:
                    ...
                    log_string = f" --> I have found {len(self.media_on_feed)} new media ID in your recent feed!\n"
                    self.write_log(log_string)
                    print()
                except:
    ...
    def write_log(self, log_text):
        ...
        if self.log_mod == 0:
            try:
                ...
                print(" --> INFO: Your text has unicode problem!\n")
        elif self.log_mod == 1:
